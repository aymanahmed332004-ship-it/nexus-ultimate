...
class ModelRouter:
    ...
    def __init__(self):
        self.models = []
        if settings.GROQ_API_KEY: self.models.append({"name": "groq", "key": settings.GROQ_API_KEY, "url": "https://api.groq.com/v1/chat/completions"})
        if settings.OPENROUTER_KEY: self.models.append({"name": "openrouter", "key": settings.OPENROUTER_KEY, "url": "https://openrouter.ai/api/v1/chat/completions"})
        if not self.models: logger.warning("⚠️ لا توجد مفاتيح API"); self.models.append({"name": "mock", "key": "", "url": ""})
    ...
    async def ask(self, prompt: str, system: str = "") -> str:
        ...
        for _ in range(len(self.models)):
            ...
            try:
                if model["name"] == "groq": return await self._call_groq(prompt, system, model)
                elif model["name"] == "openrouter": return await self._call_openrouter(prompt, system, model)
                elif model["name"] == "mock": return "رد تجريبي"
            except httpx.HTTPStatusError as e:
                if e.response.status_code == 429: logger.warning(f"Rate limit {model['name']}"); await asyncio.sleep(5); self.current_index += 1; continue
                elif e.response.status_code == 401: logger.error(f"Invalid key {model['name']}"); self.current_index += 1; continue
                else: logger.warning(f"{model['name']} failed: {e}"); self.current_index += 1; await asyncio.sleep(1)
            except Exception as e: logger.warning(f"{model['name']} failed: {e}"); self.current_index += 1; await asyncio.sleep(1)
    ...
    async def ask_stream(self, prompt: str, system: str = "") -> AsyncGenerator[str, None]:
        ...
        except Exception as e: logger.error(f"Stream error: {e}"); yield f"⚠️ خطأ: {e}"
